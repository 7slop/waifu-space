...
import { createMachiyaHouse, createCrateCluster, createStoneLantern, createSakeBarrelStack } from '../components';

/**
 * Lane A — "A-Long" West Machiya Street (X = -34..-22).
 * Long-range lane with stone gate at Z=0. The west house row is now a
 * continuous terrace (gap fillers) backed by a solid street wall, so there
 * are no walk-through gaps behind the houses leading to the map edge.
 */
export function buildLaneA(b: MapBuilder) {
  ...
  // --- West Row Terrace Fillers (seal the gaps between houses) ---
  // Turns the west row into one continuous built-up terrace: no more
  // walk-through slots between houses opening onto dead voids.
  createMachiyaHouse(b, 'mchAWF1', new Vector3(-38, 0, -18), 9, 6.3, 6.6, { hasShoji: false });
  createMachiyaHouse(b, 'mchAWF2', new Vector3(-38, 0, 0), 9, 6.4, 10.6, { hasShoji: false });
  createMachiyaHouse(b, 'mchAWF3', new Vector3(-38, 0, 18), 9, 6.3, 6.6, { hasShoji: false });

  // --- Back Street Wall (seals the void strip behind the west terrace) ---
  // Previously players could slip behind the houses and walk to the west
  // perimeter wall. This wall fuses the terrace backs to the map boundary.
  b.addBox('aBackWall', 1.2, 7.5, 74, new Vector3(-42.5, 3.75, -1), mats.wall);
  ...
  // --- East Row Terrace Fillers (seal the gaps between the east houses) ---
  createMachiyaHouse(b, 'mchAEF1', new Vector3(-22, 0, -16), 7, 6, 8.6, { hasShoji: false });
  createMachiyaHouse(b, 'mchAEF2', new Vector3(-22, 0, 0), 7, 6.2, 8.6, { hasShoji: false });
  createMachiyaHouse(b, 'mchAEF3', new Vector3(-22, 0, 16), 7, 6, 8.6, { hasShoji: false });
  ...
  // --- Back-alley props (decorative nook behind the tea house) ---
  createSakeBarrelStack(b, 'sakeBackAlley', new Vector3(-40.5, 0, -27), false);
}
