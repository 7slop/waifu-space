...
import type { BabylonMapData, BabylonSpawnPoint, MapBuilder } from './map/types';
import { createMapBuilder as createMapBuilderCore } from './map/builder';
import {
  buildLayout,
  loadDefaultLayout,
  vec3
...
/**
 * Runs every classic section builder in their original order. This is the
 * legacy programmatic map definition — used as a fallback when no kyoto.wsmap
 * is available and as the source feed for the map export script.
 */
export function buildSections(b: MapBuilder): void {
  buildGround(b);
  buildSpawns(b);
  buildLaneA(b);
  buildLaneB(b);
 ...
 *
 * The geometry is loaded from src/lib/strike/map/kyoto.wsmap (see ./map/layout).
 * When that file is missing or malformed the classic section builders run as a
 * fallback. The .wsmap file can be re-generated by editing the map in the
 * in-browser editor (launch the dev server with --edit) or by running
 * `bun run map:export` which serializes the section builders.
 */
export function createKyotoMap(scene: Scene, layout?: MapLayout): BabylonMapData {
  // ═══════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════
  // 2. BUILD MAP GEOMETRY (from .wsmap, falling back to sections)
  // ═══════════════════════════════════════════════════════════════════

  const b = createMapBuilderCore(scene, shadowGen);

  let skyMat: StandardMaterial;
  const resolvedLayout = layout ?? loadDefaultLayout();
  if (resolvedLayout) {
    buildLayout(b, resolvedLayout);
    skyMat = buildSky(b);
  } else {
    buildSections(b);
    skyMat = buildSky(b);
  ...
  let spawnPoints: BabylonSpawnPoint[];
  if (resolvedLayout && resolvedLayout.spawns.length > 0) {
    spawnPoints = resolvedLayout.spawns.map((s) => ({
